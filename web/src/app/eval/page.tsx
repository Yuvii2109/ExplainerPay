          ...
          <tbody>
            {report.metrics.map((m) => (
              <tr key={m.name}>
                ...
                <td className="dim">{m.enforcement}</td>
                <td className={m.status}>{label(m.status)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <table>
          ...
          <tbody>
            {report.rows.map((r) => (
              <tr key={r.paymentId}>
                <td>{r.paymentId}</td>
                <td>{r.expectedPath}</td>
                <td className={r.actualPath === null ? "dim" : r.pathAsExpected ? "MET" : "NOT_MET"}>
                  {r.actualPath ?? "not produced"}
                </td>
                <td className="num">{r.modelCalls}</td>
                <td className="dim">{r.injectedCause ?? "—"}</td>
                <td className={r.namedCause === null ? "dim" : r.causeCorrect ? "MET" : "NOT_MET"}>
                  {r.namedCause ?? "—"}
                </td>
              </tr>
            ))}
